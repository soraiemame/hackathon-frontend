...
// 
async function updateProfile(vars: { userId: number, data: { username: string | null } }): Promise<User> {
...
export function ProfileHeader({ user }: ProfileHeaderProps) {
    ...
    const handleProfileSave = () => {
        updateMutation.mutate({ userId: user.id, data: { username: editUsername } })
    }
                ...
                <div className="flex flex-col sm:flex-row gap-6 items-start">
                    ...
                    <div className="flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-3">
                            <div>
                                <h1 className="text-2xl font-bold mb-1">{user.username || user.email}</h1>
                                <p className="text-muted-foreground text-sm">@{user.username || '...'} | {user.created_at}から利用</p>
                            </div>
                            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>プロフィール情報を更新してください</DialogDescription>
                                    </DialogHeader>
