                    ...
                    {orders?.map((order) => (
                        <Card key={order.id}>
                            <CardContent className="pt-6">
                                <Link to={`/orders/${order.id}`} className="flex gap-4 hover:opacity-80 transition-opacity">
                                    <img
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-2 mb-2">
                                            <h3 className="font-medium line-clamp-2">{order.item.name}</h3>
                                            <Badge variant={order.status === "completed" ? "secondary" : "default"}>{order.status}</Badge>
                                        </div>
                                        <p className="text-lg font-bold text-primary mb-1">¥{order.item.price.toLocaleString()}</p>
                                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                            <span>{user && order.buyer_id === user.id ? "購入" : "販売"}</span>
                                            <span>•</span>
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">プロフィール情報の編集（上部のボタンから）</p>
                                        </div>
                                </div>
                            </div>
                            </CardContent>
